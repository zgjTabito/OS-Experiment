...
4. **观察页表解析**：
   - 如果 TLB 未命中，跟踪页表解析函数，观察页表项的加载和更新。
...
“双重 gdb”通常指：外部 gdb 附着 QEMU（调 QEMU/TCG/helper/设备模型），内部 gdbserver/remote 调试客体（guest）代码。
对于 ecall/sret 等特权指令，外层 gdb 在 helper/TCG 层能看到：
译码路径：trans_sret -> gen_helper_sret -> helper 调用点在 JIT 生成的 TB 中。
语义实现：helper_sret、riscv_raise_exception、riscv_cpu_do_interrupt。
内层 gdb（guest 视角）只能看到 guest 指令流，不直接看到 TCG/helper；遇到 ecall 会停在 guest trap 入口，sret 返回会跳回 sepc。
如果做“双重 gdb”实验，确实涉及 TCG：外层 gdb 抓到 helper 调用、TB 边界、cpu_tb_exec 等；内层 gdb 观察 guest PC/寄存器变化。两层相互印证可定位 guest 行为与 QEMU 语义实现是否一致。
